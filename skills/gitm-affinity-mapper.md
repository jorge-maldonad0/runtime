...
```
POST https://api.apify.com/v2/acts/apify~linkedin-profile-scraper/run-sync-get-dataset-items
Authorization: Bearer {APIFY_API_TOKEN}
Body: { "profileUrls": ["{linkedin_url}"] }
```

Extract: `education`, `positions`, `skills`, `summary`, `headline`, `volunteer`.
...
- `education_schools`: all school names found
- `past_employers`: all company names from positions
- `affiliations`: volunteer orgs, groups, fraternities, sororities, accelerators
- `technical_signal`: 1-2 sentence summary from headline + skills + summary
- `technical_keywords`: semicolon-separated matched keywords
